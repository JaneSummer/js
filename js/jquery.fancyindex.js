		...
		updateItems : function()
		{
			...
			var lastActive = false;
		 	...
		 	$.each( $this.items, function( key, obj )
		 	{
	 			...
	 			if( windowTop > parseInt( obj.offset.top ) + 25 )
	 			{
 					lastActive = $( obj.obj );
	 			}
		 		
		 		if( ( windowTop <= parseInt( obj.offset.top ) + 25 && windowTop + windowHeight >= parseInt( obj.offset.top ) && ( !$this.options.firstOnly || !firstFound ) ) && ( parseInt( $this.options.maxPrioritizedItems ) == 0 || focusedItems > 0 ) )
		 		{
			...
			$( this.selector.find( "h1,h2,h3,h4,h5,h6,h7" ) ).each( function( key, obj )
			{
				if( $( obj ).attr( "data-fancyindex-hide" ) != "true" )
				{
					...
					if( $this.options.scrollOnClick )
					{
						...
						{
						   offset = $( obj ).offset();
							$("html, body").animate({ scrollTop: offset.top - 120}, parseInt($this.options.scrollToDuration));
						});
					}
				}
			});
